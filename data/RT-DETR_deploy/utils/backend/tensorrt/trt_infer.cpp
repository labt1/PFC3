...
namespace trt
{
    namespace infer
    {
        ...
        //Implementación de los método de __native_engine_context
        bool __native_engine_context::construct(std::vector<unsigned char> &trtFile)
        {
            destroy();
            sample::setReportableSeverity(sample::Severity::kERROR); // Establece el nivel de salida del registro de trt. Aquí solo se generan errores.
            if (trtFile.empty())
                return false;
        ...
        // Implementación de los método de InferTRT
