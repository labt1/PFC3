    ...
    namespace memory
    {
        ...
        void *BaseMemory::gpu_realloc(size_t bytes)
        {
            if (gpu_capacity_ < bytes)
            {
                ...
                // checkRuntime(cudaMemset(gpu_, 0, size)); // Establezca los bytes de memoria solicitados por la gpu en 0, puede agregarlos o no.
            }
            gpu_bytes_ = bytes;
            return gpu_;
        }
        ...
        void *BaseMemory::cpu_realloc(size_t bytes)
        {
            if (cpu_capacity_ < bytes)
            {
                ...
                cpu_capacity_ = bytes;
                checkRuntime(cudaMallocHost(&cpu_, bytes)); // Tenga en cuenta que lo que se solicita aquí es memoria de página fija.
                Assert(cpu_ != nullptr);
                // memset(cpu_, 0, size);
            }
            cpu_bytes_ = bytes;
